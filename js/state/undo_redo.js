...
function applyAction(action) {
  ...
  if (action.type === "composite") {
    ...
    graphState.globalSettings[field] = to;
  ...
  // Émettre un événement pour notifier que l'action a été appliquée
  window.dispatchEvent(new CustomEvent('action-applied', { detail: { action } }));
}
...
export function performAction(action) {
  ...
  // Émettre un événement pour notifier que l'action a été effectuée
  window.dispatchEvent(new CustomEvent('action-performed', { detail: { action } }));
}
...
export function undo() {
  ...
  // Émettre un événement pour notifier que l'annulation a été effectuée
  window.dispatchEvent(new CustomEvent('undo-performed'));
}
...
export function redo() {
  ...
  // Émettre un événement pour notifier que le rétablissement a été effectué
  window.dispatchEvent(new CustomEvent('redo-performed'));
}
...
function updateHistoryList() {
  ...
  window.updateHistoryList?.(actionHistory);
}

// Exporter l'historique pour le débogage
